...
Python, while making advances in nearly all subdomains of computing, with an edge in deep learning, has in recent years risen as a strong contender in R's arena. Despite heightened public enthusiasm, however, it is not very likely to supplant R's dominance in data science as the go-to language. 
...
When I first learnt Python in 2006, I had to cope with the social stigma that comes with it. People in my social circle who know programming mostly use Java or C++; they paid little regard to Python, a "scripting language". Nowadays the situation is very different.
...
The statements are correct, but the conclusion is not. What's wrong with this argument?
...
Therefore, with respect to data science, Python is a lower-level programming language, and is most suitable for use beneath the friendly user interface laid out by R. Specifically, most deep learning capabilities in Python, such as _Keras_, _TensorFlow_, all have adaptations in R. 

For most people, a familarity with Python will only be useful for reading and porting existing codebases in the language. 

Using Python to approach analytical issues is like making sculpture with a chainsaw. 
![](https://upload.wikimedia.org/wikipedia/commons/8/8e/ChainsawartinBrienz.JPG)
...
In Python, 
in the follwing code example, keyword `import` 
```{python}
import numpy as np
...
### R vs. SQL
This is hardly an apples-to-apples comparison. But one can also compare an apple to an orange, both as fruits. 

SQL is at the basis of any relational database applications. 

```{sql connection=mtcars}
SELECT * FROM MTCARS
...
### R vs. SPSS
Although lacking recent buzz, SPSS is still widely and wildly popular. The proprietary software, originally named _Statistical Package for Social Sciene_, is sold by IBM. The point-and-click user interface helps generating sophisticated reports without any coding knowledge. The latest version uses Python scripting, which 

### R vs. MATLAB (GNU Octave)
