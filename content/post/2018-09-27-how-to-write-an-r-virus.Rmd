...
In the above, we saved a copy of the original function in the base package. If we didn't do that, we would have create an infinite recursion through self-refernce, such as:
```{r, eval = FALSE}
reassign("paste",
         function(...) paste(...),
         "package:base")
...
paste("foo", "bar")
```
Running this will crash the R session. If the user didn't save documents before setting off this bomb, too bad.

There are yet more ways to damages. For starters, how about deleting every `csv` file the user has in the working directory? 
```{r eval = FALSE}
...
Doing something on the file system can mean a lot.
...
Here are some futher ideas:
  ...
  -  Use Rstudio API to mess with the user's scripts
